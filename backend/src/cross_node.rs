...
impl CrossNodeVerificationEngine {
    pub async fn verify(urls: &[String]) -> CrossNodeReport {
        ...
        let (status, verification_basis) = if observations.is_empty() {
            ...
            ("pass", "observed 2/3 agreement across reachable nodes; not a stake-weighted Casper finality proof")
        } else if conflicting_nodes > 0 {
            ("warn", "conflicting finalized-block observations detected; Casper stake/validator evidence not yet established")
        } else {
            ("warn", "insufficient observed agreement or block evidence; Casper stake/validator evidence not yet established")
        };
