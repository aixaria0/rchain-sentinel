...
impl VerificationEngine {
    ...
    fn aggregate_status(checks: &[EvidenceCheck]) -> VerificationStatus {
        if checks.iter().any(|c| matches!(c.check.status, VerificationStatus::Fail)) {
            VerificationStatus::Fail
        } else if checks.iter().any(|c| matches!(c.check.status, VerificationStatus::Warn)) {
    ...
    fn finalized_block_cross_check(
        ...
        if !evidence.available {
            return Self::check(
                "finalized_block_cross_check", VerificationStatus::Warn,
                evidence.error.as_deref().unwrap_or("Independent finalized-block evidence is unavailable."),
                CheckSeverity::Warning, "last-finalized-block", "available", "false",
            );
        }
        ...
        let raw = match &evidence.raw {
            ...
            None => return Self::check(
                ...
                CheckSeverity::Warning, "last-finalized-block", "payload", "missing",
            ),
        };

        let evidence_height = Self::extract_block_height(raw);
        match (status_height, evidence_height) {
            (Some(a), Some(b)) if a == b => Self::check(
                "finalized_block_cross_check", VerificationStatus::Pass,
                &format!("Finalized block height matches across independent RNode evidence paths: {}.", a),
                CheckSeverity::Info, "cross-check", "finalized_block_height",
                format!("status={}, evidence={}", a, b),
            ),
            (Some(a), Some(b)) => Self::check(
                "finalized_block_cross_check", VerificationStatus::Fail,
                &format!("Finalized block height mismatch: status reports {}, while last-finalized-block reports {}.", a, b),
                CheckSeverity::Critical, "cross-check", "finalized_block_height",
                format!("status={}, evidence={}", a, b),
            ),
            (Some(a), None) => Self::check(
                "finalized_block_cross_check", VerificationStatus::Warn,
                &format!("RNode status reports finalized block {}, but no comparable block height could be extracted from the independent evidence.", a),
                CheckSeverity::Warning, "cross-check", "status_finalized_block", a.to_string(),
            ),
            (None, Some(b)) => Self::check(
                "finalized_block_cross_check", VerificationStatus::Warn,
                &format!("Independent evidence reports finalized block {}, but RNode status does not expose a comparable height.", b),
                CheckSeverity::Warning, "cross-check", "evidence_finalized_block", b.to_string(),
            ),
            (None, None) => Self::check(
                "finalized_block_cross_check", VerificationStatus::Warn,
                "Both verification paths lack a comparable finalized block height.",
                CheckSeverity::Warning, "cross-check", "finalized_block_height", "unavailable",
            ),
        }
    }
        ...
        match value {
            ...
            Value::Object(map) => {
                for key in ["blockNumber", "block_number", "height", "blockHeight", "block_height", "seqNum", "seq_num"] {
                    if let Some(v) = map.get(key).and_then(Self::value_as_u64) {
                        return Some(v);
                    }
                }
                for key in ["block", "header", "metadata"] {
                    if let Some(v) = map.get(key).and_then(Self::extract_block_height) {
                        return Some(v);
                    }
                }
                None
    ...
    fn reachability(status: &NetworkStatus) -> EvidenceCheck {
        if status.reachable {
            Self::check("node_reachable", VerificationStatus::Pass, "Node responded to the verification probe.", CheckSeverity::Info, "rnode", "reachable", status.reachable.to_string())
        } else {
            Self::check("node_reachable", VerificationStatus::Fail, status.error.as_deref().unwrap_or("Node did not respond."), CheckSeverity::Critical, "rnode", "reachable", status.reachable.to_string())
        }
    }
    ...
    fn probe_integrity(status: &NetworkStatus) -> EvidenceCheck {
        let valid = status.probe.starts_with("http://") || status.probe.starts_with("https://");
        if valid {
            Self::check("probe_integrity", VerificationStatus::Pass, "Verification probe uses a valid HTTP(S) endpoint.", CheckSeverity::Info, "sentinel", "probe", status.probe.clone())
        } else {
            Self::check("probe_integrity", VerificationStatus::Fail, "Verification probe is not a valid HTTP(S) endpoint.", CheckSeverity::Critical, "sentinel", "probe", status.probe.clone())
        }
    }

    fn rnode_payload_presence() -> EvidenceCheck {
        Self::check("rnode_payload", VerificationStatus::Warn, "Node responded, but no structured RNode status payload was available.", CheckSeverity::Warning, "rnode", "payload", "missing")
    }
    ...
    fn network_identity(o: &RNodeObservation) -> EvidenceCheck {
        match (o.network_id.as_deref(), o.shard_id.as_deref()) {
            (Some(network), Some(shard)) => Self::check("network_identity", VerificationStatus::Pass, "Network and shard identity are available.", CheckSeverity::Info, "observation", "network/shard", format!("network_id={}, shard_id={}", network, shard)),
            (Some(network), None) => Self::check("network_identity", VerificationStatus::Warn, "Network identity is available but shard identity is missing.", CheckSeverity::Warning, "observation", "network_id", network),
            (None, Some(shard)) => Self::check("network_identity", VerificationStatus::Warn, "Shard identity is available but network identity is missing.", CheckSeverity::Warning, "observation", "shard_id", shard),
    ...
    fn observation_integrity(o: &RNodeObservation) -> EvidenceCheck {
        let complete = o.node_id.is_some()
            && o.network_id.is_some()
            && (o.ready.is_some() || o.validator.is_some() || o.finalized_block.is_some());
        if complete {
            Self::check("observation_integrity", VerificationStatus::Pass, "Observation contains identity, network, and node-state evidence.", CheckSeverity::Info, "observation", "integrity", "complete")
        } else {
            Self::check("observation_integrity", VerificationStatus::Warn, "Observation is incomplete and cannot provide a full node-state picture.", CheckSeverity::Warning, "observation", "integrity", "partial")
        }
    }
    ...
    fn check(
        ...
        EvidenceCheck {
            check: VerificationCheck { name: name.to_string(), status, message: message.to_string(), severity, evidence: vec![VerificationEvidence { source: source.to_string(), field: field.to_string(), value: value.into() }] },
            severity: match &status {
                VerificationStatus::Fail => CheckSeverity::Critical,
                VerificationStatus::Warn => CheckSeverity::Warning,
                VerificationStatus::Pass => CheckSeverity::Info,
            },
            evidence: vec![],
        }
    }
    ...
    fn check_without_evidence(
        ...
        EvidenceCheck {
            check: VerificationCheck { name: name.to_string(), status, message: message.to_string(), severity: match &status { VerificationStatus::Fail => CheckSeverity::Critical, VerificationStatus::Warn => CheckSeverity::Warning, VerificationStatus::Pass => CheckSeverity::Info }, evidence: Vec::new() },
            severity,
            evidence: Vec::new(),
        }
    }
