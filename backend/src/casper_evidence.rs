...
impl CasperEvidenceEngine {
    pub fn analyze(evidence: &FinalizedBlockEvidence) -> CasperEvidenceReport {
        ...
        let validator_identity_present = Self::contains_any(raw, &["validator", "validatorId", "validator_id", "proposer", "sender"]);
        let stake_weight_present = Self::contains_any(raw, &["stake", "weight", "stakeWeight", "stake_weight", "validatorStake", "validator_stake"]);
        let bet_present = Self::contains_any(raw, &["bet", "bets", "belief", "beliefs", "proposition", "propositions", "claim", "source", "target"]);
        let justification_present = Self::contains_any(raw, &["justification", "justifications", "justificationMap", "justification_map"]);
        ...
        let recognized_fields = recognized.into_iter().filter(|key| Self::is_casper_key(key)).collect::<Vec<_>>();

        let sufficient_for_stake_analysis = validator_identity_present && stake_weight_present;
        let status = if equivocation_signal { "warning" } else if sufficient_for_stake_analysis && bet_present && justification_present { "observed" } else { "insufficient" };
    ...
    fn contains_any(value: &Value, keys: &[&str]) -> bool {
        match value {
            Value::Object(map) => keys.iter().any(|key| map.contains_key(*key)) || map.values().any(|nested| Self::contains_any(nested, keys)),
    ...
    fn collect_keys(value: &Value, keys: &mut BTreeSet<String>) {
        match value {
            Value::Object(map) => { for (key, nested) in map { keys.insert(key.clone()); Self::collect_keys(nested, keys); } }
            Value::Array(items) => { for item in items { Self::collect_keys(item, keys); } }
            _ => {}
        }
    }
    ...
    fn is_casper_key(key: &str) -> bool {
        ...
        ["validator", "validatorid", "validator_id", "proposer", "sender", "stake", "weight", "stakeweight", "stake_weight", "validatorstake", "validator_stake", "bet", "bets", "belief", "beliefs", "proposition", "propositions", "claim", "source", "target", "justification", "justifications", "justificationmap", "justification_map", "equivocation", "equivocated", "doublevote", "double_vote", "conflictingbets", "conflicting_bets"].contains(&key.as_str())
    }
}
